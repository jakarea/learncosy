                    ...
                    <form action="" method="POST" id="objectForm">
                        ...
                        <div class="content-settings-form-wrap">
                                ...
                                @if (!empty($course->objective))
                                    @foreach (explode('[objective]', $course->objective) as $index => $objective)
                                        <div class="item">
                                            ...
                                            <input autocomplete="off" type="text" value="{{ $objective }}"
                                                class="form-control" disabled>
                                            <div class="actions">
                                                <a href="#" class="me-2 edit-item" data-index="{{ $index }}">
                                                    <img src="{{ asset('latest/assets/images/icons/pen-m.svg') }}"
                                                        alt="Edit" class="img-fluid">
                                                </a>
                                                <a href="#" class="delete-item" data-index="{{ $index }}">
                                                    <img src="{{ asset('latest/assets/images/icons/minus-m.svg') }}"
                            ...
                            <div class="form-group">
                                <h6>Object Title</h6>
                                <textarea class="form-control" name="objective[]" placeholder="Enter objective" id="objective" required>{{ old('objective') }}</textarea>
                            </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            editItem.forEach(item => {
                item.addEventListener('click', function(e) {
                    ...
                    objectiveTextArea.value = item.parentNode.parentNode.querySelector('input')
                        .value;
                    ...
                    fetch(`${baseUrl}/instructor/courses/create/${courseId}/objects`, {
                        ...
                        .then(data => {
                            if (data.message === 'ADDED') {
                                ...
                                newItem.innerHTML = `
                                        <i class="fas fa-check"></i>
                                        <input autocomplete="off" type="text" value="${objectiveTextArea.value}" class="form-control" disabled>
                                        <div class="actions">
                                ...
                                updatedItems.forEach(itm => {
                                    if (itm.getAttribute('data-index') == dataIndex) {
                                        itm.parentNode.parentNode.querySelector('input').value =
                                            objectiveTextArea.value;
                                    }
                                });
